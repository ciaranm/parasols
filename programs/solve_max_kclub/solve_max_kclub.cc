...
#include <max_kclub/algorithms.hh>
...
auto main(int argc, char * argv[]) -> int
{
    try {
        ...
        if (options_vars.count("help")) {
            std::cout << "Usage: " << argv[0] << " [options] algorithm order k file[...]" << std::endl;
            std::cout << std::endl;
        ...
        if (! options_vars.count("algorithm") || options_vars.count("input-file") < 1 || ! options_vars.count("k")) {
            std::cout << "Usage: " << argv[0] << " [options] algorithm order k file[...]" << std::endl;
            return EXIT_FAILURE;
        }

        /* Turn an algorithm string name into a runnable function. */
        auto algorithm = max_kclub_algorithms.begin(), algorithm_end = max_kclub_algorithms.end();
        ...
        if (algorithm == algorithm_end) {
            ...
            for (auto a : max_kclub_algorithms)
        ...
        for (auto & input_file : input_files) {
            ...
            MaxKClubParams params;

            params.order_function = order_function;
            params.k = options_vars["k"].as<int>();
